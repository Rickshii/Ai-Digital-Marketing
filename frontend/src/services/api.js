...
import { DEFAULT_QR_BASE64 } from '../assets/defaultQRBase64';
  ...
  getQRUrl: async () => {
    return executeWithFallback(
      async () => {
        ...
        return url || DEFAULT_QR_BASE64;
      },
      () => {
        // Fallback: use cached URL, then bundled default QR image
        const cached = localStorage.getItem('mock_qr_url');
        return cached || DEFAULT_QR_BASE64;
      }
    );
  },
};
...
export const adminAPI = {
  ...
  getQRUrl: async () => {
    return executeWithFallback(
      ...
      () => localStorage.getItem('mock_qr_url') || DEFAULT_QR_BASE64
    );
  },
