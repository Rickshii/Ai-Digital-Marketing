...
import { Sparkles, Mail, Lock, Eye, EyeOff, ArrowRight, TrendingUp, BarChart2, Globe, Zap, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
const Login = () => {
  ...
  const handleForgotSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await authAPI.forgotPassword(resetEmail, newPassword);
      setSuccessMsg(res.detail || 'Password reset successfully.');
      setTimeout(() => {
        setView('login');
        setEmail(resetEmail);
        setPassword('');
            ...
            {view === 'login' ? (
              ...
              >
                <div className="mb-8">
                  ...
                  <h1 className="text-2xl font-extrabold text-slate-800">Reset password</h1>
                  <p className="text-slate-500 text-sm mt-1">Enter your email and a new password to recover your account.</p>
                </div>
                ...
                {successMsg && (
                  <div className="mb-5 rounded-xl border border-emerald-100 bg-emerald-50 p-3 text-sm text-emerald-700 flex items-start gap-2">
                ...
                <form onSubmit={handleForgotSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500 mt-1.5 ml-1">Must be at least 6 characters long.</p>
                  </div>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full mt-2 flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-500/25 hover:shadow-violet-500/40 hover:opacity-95 transition-all disabled:opacity-60"
                  >
                    {loading ? (
                      <div className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    ) : (
