...
def get_admin_stats(
    ...
    active_users = db.query(UserModel).filter(
        UserModel.created_at >= datetime.utcnow() - timedelta(days=30)
    ).count()
    new_registrations = db.query(UserModel).filter(
    ...
    for i in range(5, -1, -1):
        ...
        months_stats.append({"month": start_date.strftime("%b"), "count": count})
    ...
    return {
        ...
        "website_audits": website_audits_count,
...
def list_users(
    ...
    for u in users:
        ...
        profile_data = [{"id": p.id, "business_name": p.business_name,
                         "business_category": p.business_category, "website_url": p.website_url}
                        for p in profiles]
        ...
        payment_list = [{"id": p.id, "amount": p.amount, "razorpay_order_id": p.razorpay_order_id,
                         "razorpay_payment_id": p.razorpay_payment_id, "status": p.status,
        ...
        user_list.append({
            ...
            "created_at": u.created_at, "audits_count": audits_count,
            "reports_count": reports_count, "business_profiles": profile_data,
            "access": access, "payments": payment_list,
        })
    return user_list
...
def preview_user(
    ...
    profile_data = [{"id": p.id, "business_name": p.business_name,
                     "business_category": p.business_category, "website_url": p.website_url,
                     "industry_type": p.industry_type, "description": p.description,
                     "email": p.email, "contact_number": p.contact_number,
    ...
    return {
        ...
        "created_at": user.created_at, "audits_count": audits_count,
        "reports_count": reports_count, "business_profiles": profile_data,
        "access": access, "payments": payment_list,
    }
...
def update_user(
    ...
    return {"id": user.id, "email": user.email, "full_name": user.full_name,
            "role": user.role, "detail": "User updated successfully."}
...
def list_all_reports(
    ...
    for r in reports:
        ...
        reports_list.append({
            "id": r.id, "report_id": r.report_id, "title": r.title, "type": r.type,
            "created_at": r.created_at, "scores": r.scores,
...
def list_plans(
    ...
    """List ALL subscription plans (including special). Seeds defaults if empty."""
    ...
    return [
        {
            "id": p.id, "plan_name": p.plan_name, "price": p.price,
            "duration_days": p.duration_days, "description": p.description,
            "is_special": p.is_special, "updated_at": p.updated_at,
        }
        for p in plans
    ]
...
def create_plan(
    ...
    """Create a new subscription plan. Set is_special=true for admin-only plans."""
    ...
    new_plan = PlanPrice(
        ...
        is_special=body.is_special or False,
    )
    db.add(new_plan)
    db.commit()
    db.refresh(new_plan)
    return {
        "id": new_plan.id, "plan_name": new_plan.plan_name, "price": new_plan.price,
        "duration_days": new_plan.duration_days, "description": new_plan.description,
        "is_special": new_plan.is_special, "updated_at": new_plan.updated_at,
...
def update_plan(
    ...
    """Update price / duration / description / is_special flag."""
    ...
    plan.is_special = body.is_special if body.is_special is not None else plan.is_special
    plan.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(plan)
    return {
        "id": plan.id, "plan_name": plan.plan_name, "price": plan.price,
        "duration_days": plan.duration_days, "description": plan.description,
        "is_special": plan.is_special, "updated_at": plan.updated_at,
...
# ─── Assign Special Plan to a User ───────────────────────────────────────────

class AssignPlanRequest(BaseModel):
    plan_id: int


@router.post("/users/{user_id}/assign-plan", response_model=Dict[str, Any])
def assign_plan_to_user(
    ...
    """Admin-only: assign any plan (especially special plans) to a user without payment."""
    user = db.query(UserModel).filter(UserModel.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")

    plan = db.query(PlanPrice).filter(PlanPrice.id == body.plan_id).first()
    if not plan:
        raise HTTPException(status_code=404, detail="Plan not found.")

    result = AccessService.assign_special_plan(
        db=db,
        user_id=user_id,
    ...
    return {
        "success": True,
        "detail": f"Plan '{plan.plan_name}' assigned to {user.email}.",
